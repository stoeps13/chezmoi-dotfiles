...
for i in $(sudo virsh list --all | grep -e 'cnx8cr4-ora' | awk '/shut off/{print $2}')
do
  sudo virsh start $i
done
...
echo
ssh oracle@cnx8cr4-ora-db.stoeps.home /home/oracle/scripts/ora_start.sh
echo
echo "  +++++ Starting Connections +++++"
echo
ssh root@cnx8cr4-ora-was.stoeps.home ./cnx-start.sh
echo
echo "  +++++ Restart Haproxy and nginx +++++"
echo
ssh root@cnx8cr4-ora.stoeps.home 'systemctl restart haproxy'
ssh root@cnx8cr4-ora.stoeps.home 'systemctl restart nginx'
