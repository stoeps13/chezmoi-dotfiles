...
for i in $(sudo virsh list --all | grep -e 'cnx7-db2' | awk '/shut off/{print $2}')
do
  sudo virsh start $i
done
...
echo
ssh root@cnx7-db2-was.stoeps.home ./cnx-start.sh
echo
echo "  +++++ Restart Haproxy and nginx +++++"
echo
ssh root@cnx7-db2.stoeps.home 'systemctl restart haproxy'
ssh root@cnx7-db2.stoeps.home 'systemctl restart nginx'
